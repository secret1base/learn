...
public class A165_CompareVersionNumbers {
     ...
     */
    /**
     * 			执行耗时:1 ms,击败了82.64% 的Java用户
     * 			内存消耗:36.5 MB,击败了76.72% 的Java用户
     * @param version1
     * @param version2
     * @return
     */
    public int compareVersion(String version1, String version2) {
        ...
        while (x<arr1.length||y<arr2.length){
            int a= x<arr1.length?Integer.parseInt(arr1[x]):0;
            int b= y<arr2.length?Integer.parseInt(arr2[y]):0;
            if(a<b){
                return -1;
            }else if(a>b){
                return 1;
    ...
    public static void main(String[] args) {
        int i = new A165_CompareVersionNumbers().compareVersion("1.1.0", "1");
